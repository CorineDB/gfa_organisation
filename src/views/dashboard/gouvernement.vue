    ...
    <Modal :show="deleteModalPreview" @hidden="deleteModalPreview = false">
      <ModalBody class="p-0">
        ...
        <div class="px-5 pb-8 text-center">
          ...
          <button type="button" @click="deleteGroupe" class="btn btn-danger w-24">Supprimer</button>
        </div>
      </ModalBody>
    </Modal>
    <!-- END: Modal Content -->
        ...
        <tbody>
          <tr v-for="(data, index) in resultQuery" :key="index">
            ...
            <td class="flex space-x-2 items-center">
              ...
              <Tippy tag="a" href="javascript:;" class="tooltip btn btn-secondary w-24 mr-1 mb-2" content="cliquez pour ajouter ou voir les principes">
                <span @click="voirPrincipe(index, data.id)" class="text-blue-500 cursor-pointer">Principes</span>
              </Tippy>
...
const storeGroupe = function () {
  if (chargement.value == false) {
    chargement.value = true;
    GouvernanceService.create(formData)
      .then((data) => {
        message.type = "success";
        message.message = "Nouveaux groupe créee";
        successNotificationToggle();
...
const deleteGroupe = function () {
  deleteModalPreview.value = false;
  groupes.value.splice(groupes.value.indexOf(deleteData.index), 1);
  GouvernanceService.destroy(deleteData.id)
    .then((data) => {
      message.type = "success";
      message.message = "Operation éffectué avec success";
      successNotificationToggle();
...
const updateGroupe = function () {
  if (chargement.value == false) {
    ...
    GouvernanceService.update(saveUpdate.id, formData)
      .then((data) => {
        chargement.value = false;
        message.type = "success";
        message.message = "Mise à jours éffectué avec succèss";
