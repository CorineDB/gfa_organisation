    ...
    <Modal :show="deleteModalPreview" @hidden="deleteModalPreview = false">
      <ModalBody class="p-0">
        ...
        <div class="px-5 pb-8 text-center">
          ...
          <button type="button" @click="deletePrincipe" class="btn btn-danger w-24">
            Supprimer
          </button>
        </div>
      </ModalBody>
...
const getData = function () {
  PrincipeService.get(route.params.id).then((data) => {
...
const getGouvernance = function () {
  GouvernanceService.getGouvernanceByEntreprise().then((data) => {
...
const storePrincipe = function () {
  if (chargement.value == false) {
    ...
    PrincipeService.create(formData).then((data) => {
...
const deletePrincipe = function () {
  ...
  PrincipeService.destroy(deleteData.id).then((data) => {
    message.type = 'success'
...
const updatePrincipe = function () {
  ...
  if (chargement.value == false) {
    ...
    PrincipeService.update(saveUpdate.principe_id, formData).then((data) => {
      chargement.value = false
      message.type = 'success'
