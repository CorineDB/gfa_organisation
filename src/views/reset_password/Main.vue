          ...
          <form @submit.prevent="ressetPassword" class="flex items-center">
            <div class="w-full">
              ...
              <div class="mt-5 text-center intro-x xl:mt-8 xl:text-left">
                <VButton :loading="chargement" label="Envoyer" class="py-3" :disabled="score1 < 5 || score2 < 5" />
              </div>
            </div>
          </form>
        </div>
...
// vérifier les mots de passe sont forts:
const options = {
  dictionary: {
...
const score1 = computed(() => {
  if (!payload.new_password || payload.new_password.length === 0) {
    return 0;
  }
  return zxcvbn(payload.new_password).score + 1;
});

const score2 = computed(() => {
  if (!payload.new_password_confirmation || payload.new_password_confirmation.length === 0) {
    return 0;
  }
  return zxcvbn(payload.new_password_confirmation).score + 1;
});
